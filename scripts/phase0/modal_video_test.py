...
def process_video_test(max_frames: int = 100) -> dict:
    ...
    with Timer("Total GPU time", logger, level="debug") as total_timer:
        ...
        with Timer("Frame extraction", logger) as extraction_timer:
            frames = extract_frames(video_path, frames_dir, fps=1)
        extraction_time = extraction_timer.elapsed or 0.0
        frame_paths = [f.path for f in frames]
        total_frames = len(frames)
        logger.info("Extracted %d frames in %.2fs", total_frames, extraction_time)

        # Limit frames for embedding test
        frames_to_embed = frame_paths[:max_frames]
        logger.info(
            "Will embed %d frames (max_frames=%d)", len(frames_to_embed), max_frames
        )

        # Step 2: Load model
        logger.info("Loading Qwen3-VL-Embedding model...")
        with Timer("Model load", logger) as model_timer:
            model = Qwen3VLEmbedder(model_name_or_path="Qwen/Qwen3-VL-Embedding-2B")
        model_load_time = model_timer.elapsed or 0.0
        logger.info("Model loaded in %.2fs", model_load_time)

        # Step 3: Embed frames one at a time (to avoid OOM)
        logger.info(
            "Embedding %d frames (one at a time)...", len(frames_to_embed)
        )
        with Timer("Embedding frames", logger) as embed_timer:
            all_embeddings = []
            for i, frame_path in enumerate(frames_to_embed):
                ...
                if (i + 1) % 10 == 0:
                    logger.info(
                        "Embedded %d/%d frames...", i + 1, len(frames_to_embed)
                    )
        ...
        embed_time_total = embed_timer.elapsed or 0.0
        embed_time_per_frame = embed_time_total / len(frames_to_embed)
        logger.info(
            "Embedded in %.2fs (%.3fs/frame)", embed_time_total, embed_time_per_frame
        )
    ...
    total_gpu_time = total_timer.elapsed or 0.0
...
def main(max_frames: int = 100):
    ...
    if not LOCAL_VIDEO_PATH.exists():
        ...
        logger.error("Download a video first:")
        logger.error(
            '  uv run yt-dlp -f "best[height<=720]" -o "test_video.mp4" "YOUTUBE_URL"'
        )
        return
    ...
    logger.info("Using video: %s", LOCAL_VIDEO_PATH)
    logger.info("Max frames: %d", max_frames)
    ...
    if results["gate_passed"]:
        logger.info("GATE CHECK: PASSED - Extrapolated 30-min cost < $1")
    else:
        logger.error("GATE CHECK: FAILED - Extrapolated 30-min cost >= $1")
        logger.error("Batching optimization may be required.")
    logger.info("=" * 60)
