...
def test_get_query_embedder_class_requires_complete_modal_token_pair(monkeypatch) -> None:
    modal_config.get_query_embedder_class.cache_clear()
    ...
    with pytest.raises(modal_config.ModalAuthError, match="Incomplete Modal credentials"):
        modal_config.get_query_embedder_class()
