...
uv run vmf keys revoke <key_id> --bearer-token <your-clerk-bearer-token>
```
...
The CLI uses the reliable direct-upload API flow under the hood:
...
| `vmf videos upload` | `POST` | `/api/v1/videos/upload/init` | API key |
| `vmf videos upload` | `PUT` | presigned `upload_url` | presigned URL |
| `vmf videos upload` | `POST` | `/api/v1/videos/upload/complete` | API key |
